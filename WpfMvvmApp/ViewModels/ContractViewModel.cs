...
namespace WpfMvvmApp.ViewModels
{
    public class ContractViewModel : INotifyPropertyChanged, IDataErrorInfo
    {
        ...
        private void ExecuteBillSelectedLessons(object? parameter) { if (parameter is not IList selectedItems || selectedItems.Count == 0 || !CanExecuteBillSelectedLessons(parameter)) return; var lessonsToBill = selectedItems.OfType<Lesson>().Where(l => l.IsConfirmed && !l.IsBilled).ToList(); if (!lessonsToBill.Any()) { MessageBox.Show("No confirmed, unbilled lessons selected.", "Billing", MessageBoxButton.OK, MessageBoxImage.Information); return; } var inputDialog = new InvoiceInputDialog { Owner = Application.Current.MainWindow }; if (inputDialog.ShowDialog() == true) { string invoiceNumber = inputDialog.InvoiceNumber; DateTime? invoiceDate = inputDialog.InvoiceDate; if (!invoiceDate.HasValue) { MessageBox.Show("Invoice date was not correctly selected.", "Internal Error", MessageBoxButton.OK, MessageBoxImage.Error); return; } foreach (var lesson in lessonsToBill) { lesson.IsBilled = true; lesson.InvoiceNumber = invoiceNumber; lesson.InvoiceDate = invoiceDate; } NotifyBillingRelatedChanges(); (BillSelectedLessonsCommand as RelayCommand)?.RaiseCanExecuteChanged(); MessageBox.Show($"{lessonsToBill.Count} lessons marked as billed with Invoice # {invoiceNumber} dated {invoiceDate.Value:d}.", "Billing Complete", MessageBoxButton.OK, MessageBoxImage.Information); } }
