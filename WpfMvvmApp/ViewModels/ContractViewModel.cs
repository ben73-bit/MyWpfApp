...
using WpfMvvmApp.Properties; // AGGIUNTO using per Resources
...
namespace WpfMvvmApp.ViewModels
{
    public class ContractViewModel : INotifyPropertyChanged, IDataErrorInfo
    {
        ...
        public decimal? HourlyRate { get => Contract?.HourlyRate; set { if (Contract != null && Contract.HourlyRate != value) { Contract.HourlyRate = value; OnPropertyChanged(); OnPropertyChanged(nameof(IsContractValid)); NotifyBillingRelatedChanges(); } } }
        ...
        public string NewLessonStartTimeString
        {
            ...
            set { if (SetProperty(ref _newLessonStartTimeString, value)) { OnPropertyChanged(nameof(IsLessonInputValid)); (AddLessonCommand as RelayCommand)?.RaiseCanExecuteChanged(); } }
        }
        public TimeSpan NewLessonDuration
        {
             get => _newLessonDuration;
             set { if (SetProperty(ref _newLessonDuration, value)) { OnPropertyChanged(nameof(IsLessonInputValid)); (AddLessonCommand as RelayCommand)?.RaiseCanExecuteChanged(); NotifyBillingRelatedChanges(); } }
        }
        ...
        // LoadLessons
        private void LoadLessons() { Lessons.Clear(); if (_contract?.Lessons != null) { foreach (var lesson in _contract.Lessons.OrderBy(l=>l.StartDateTime)) { Lessons.Add(lesson); } } NotifyBillingRelatedChanges(); }
        ...
        string IDataErrorInfo.this[string columnName]
        {
            get
            {
                // CORRETTO: Usa risorsa per messaggio errore
                if (columnName == nameof(NewLessonStartTimeString)) { if (!TryParseTime(NewLessonStartTimeString, out _)) return Resources.Validation_InvalidTimeFormat ?? "Invalid Start Time format (use HH:MM)."; }
                return string.Empty;
            }
        }
        private string GetFirstError() { if (!TryParseTime(NewLessonStartTimeString, out _)) return Resources.Validation_InvalidTimeFormat ?? "Invalid Start Time format."; return string.Empty; } // Usa risorsa
        ...
        private void ExecuteAddOrUpdateLesson(object? parameter)
        {
            ...
            if (IsEditingLesson && _lessonToEdit != null) { var oldDuration = _lessonToEdit.Duration; _lessonToEdit.StartDateTime = finalStartDateTime; _lessonToEdit.Duration = NewLessonDuration; _lessonToEdit.Summary = NewLessonSummary; if (oldDuration != _lessonToEdit.Duration) requiresNotify = true; }
            else { var newLesson = new Lesson { StartDateTime = finalStartDateTime, Duration = NewLessonDuration, Contract = Contract, IsConfirmed = false, Summary = NewLessonSummary }; Lessons.Add(newLesson); Contract.Lessons ??= new List<Lesson>(); Contract.Lessons.Add(newLesson); requiresNotify = true; }
        ...
        private void ExecuteRemoveLesson(object? parameter)
        {
             ...
             {
                 // CORRETTO: Usa risorse
                 string message = string.Format(Resources.MsgBox_ConfirmRemoveLesson_Text ?? "Remove lesson starting {0:g}?", lessonToRemove.StartDateTime);
                 var result = MessageBox.Show(message, Resources.MsgBox_Title_ConfirmRemoval ?? "Confirm", MessageBoxButton.YesNo, MessageBoxImage.Warning);
                 if (result == MessageBoxResult.Yes) { bool removed = Lessons.Remove(lessonToRemove); Contract.Lessons?.Remove(lessonToRemove); if (removed) { NotifyBillingRelatedChanges(); } }
             }
        }
        private void ExecuteRemoveSelectedLessons(object? parameter)
        {
             ...
             // CORRETTO: Usa risorse
             string message = selectedItems.Count == 1 ? Resources.MsgBox_ConfirmRemoveSelectedLesson_Text_Singular ?? "Remove selected lesson?" : string.Format(Resources.MsgBox_ConfirmRemoveSelectedLessons_Text_Plural ?? "Remove {0} selected lessons?", selectedItems.Count);
             ...
             if (result == MessageBoxResult.Yes) { bool lessonsRemoved = false; var lessonsToRemove = selectedItems.OfType<Lesson>().ToList(); foreach (var lesson in lessonsToRemove) { bool removed = Lessons.Remove(lesson); Contract.Lessons?.Remove(lesson); if (removed) { lessonsRemoved = true; } } if (lessonsRemoved) { NotifyBillingRelatedChanges(); } }
        }
        ...
        private void ExecuteImportLessons(object? parameter)
        {
            ...
            try
            {
                ...
                if (imported.Count == 0) { MessageBox.Show(Resources.MsgBox_NoLessonsToImport_Text ?? "No lessons found.", Resources.MsgBox_Title_ImportResult ?? "Import", MessageBoxButton.OK, MessageBoxImage.Information); return; }
                int addedCount = 0;
                Contract.Lessons ??= new List<Lesson>();
                foreach (var lesson in imported) { lesson.Contract = Contract; Lessons.Add(lesson); Contract.Lessons.Add(lesson); addedCount++; }
                if (addedCount > 0) { NotifyBillingRelatedChanges(); MessageBox.Show(string.Format(Resources.MsgBox_ImportSuccessful_Text ?? "{0} lessons imported.", addedCount), Resources.MsgBox_Title_ImportComplete ?? "Import Complete", MessageBoxButton.OK, MessageBoxImage.Information); }
            }
        ...
        // CORRETTO: Logica CanExecute Bill
        private bool CanExecuteBillSelectedLessons(object? parameter)
        {
            ...
            {
                 return selectedItems.OfType<Lesson>().Any(l => l.IsConfirmed && !l.IsBilled);
            }
            return false;
        }
        ...
        // CORRETTO: Assicura assegnazione a 'result'
        private bool TryParseTime(string? input, out TimeSpan result)
        {
            result = TimeSpan.Zero; // Assegna default
